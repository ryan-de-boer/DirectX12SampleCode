...
const UINT streamOutputDataSize = 120; //4*3
const UINT bufferSize = streamOutputDataSize + sizeof(UINT64); //40*3+8
const UINT strideSize = 40;

// Assume desired alignment is 256 bytes (adjust as needed)
const UINT alignment = 256;

// Calculate the padding needed to align the buffer size
const UINT padding = (alignment - (bufferSize % alignment)) % alignment;

// Calculate the total buffer size including padding
const UINT totalBufferSize = bufferSize + padding;
...
bool InitD3D()
{
	...
	rootSignatureDesc.Init(0, nullptr, 0, nullptr, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT| D3D12_ROOT_SIGNATURE_FLAG_ALLOW_STREAM_OUTPUT);
	...
	D3D12_INPUT_ELEMENT_DESC inputLayout[] =
	{
			...
			{ "WORLDPOS", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 12, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
			{ "SV_POSITION", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 24, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 }
	};
	...
	// Allocate memory for the stream output declaration
	D3D12_SO_DECLARATION_ENTRY* pSODeclaration = new D3D12_SO_DECLARATION_ENTRY[3];
	streamOutputDesc.NumEntries = 3;

	// Define the stream output declaration for position and world position
	pSODeclaration[0].Stream = 0;
	pSODeclaration[0].SemanticName = "POSITION";
	pSODeclaration[0].SemanticIndex = 0;
	pSODeclaration[0].StartComponent = 0;
	pSODeclaration[0].ComponentCount = 3;
	pSODeclaration[0].OutputSlot = 0;

	pSODeclaration[1].Stream = 0;
	pSODeclaration[1].SemanticName = "WORLDPOS";
	pSODeclaration[1].SemanticIndex = 0;
	pSODeclaration[1].StartComponent = 0;
	pSODeclaration[1].ComponentCount = 3;
	pSODeclaration[1].OutputSlot = 0;

	pSODeclaration[2].Stream = 0;
	pSODeclaration[2].SemanticName = "SV_POSITION";
	pSODeclaration[2].SemanticIndex = 0;
	pSODeclaration[2].StartComponent = 0;
	pSODeclaration[2].ComponentCount = 4;
	pSODeclaration[2].OutputSlot = 0;


	streamOutputDesc.pSODeclaration = pSODeclaration;

	// Set the buffer stride
	streamOutputDesc.NumStrides = 1;
	UINT* pBufferStrides = new UINT[1];
	pBufferStrides[0] = strideSize; // Buffer stride (size of each vertex in bytes)
	...
	// a triangle
	Vertex vList[] = {
		{ { 0.0f, 0.5f, 0.5f }, {0.0f, 0.5f, 0.5f, 1.0f}, {0.0f, 0.5f, 0.5f, 1.0f} },
		{ { 0.5f, -0.5f, 0.5f }, { 0.5f, -0.5f, 0.5f, 1.0f }, { 0.5f, -0.5f, 0.5f, 1.0f } },
		{ { -0.5f, -0.5f, 0.5f }, { -0.5f, -0.5f, 0.5f, 1.0f }, { -0.5f, -0.5f, 0.5f, 1.0f } },
	};
	...
	device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT)/* heap properties */, 
		...
		IID_PPV_ARGS(&streamOutputBuffer));
